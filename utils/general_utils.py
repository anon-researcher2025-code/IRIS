...
from models.iris_api import InterpretableRiskClusteringIntelligence 
...
def train_test_IRIS(param, X_train, X_test, orig_X_test, y_train, y_test, feature_names, dataset_name, optuna, seed=420, fix=False, method='IRIS'):
    print('param: ', param)
    ...
    num_units = get_num_units(units_multiplier, num_basis_functions, X_train)
    print('num_units: ', len(num_units))

    model = InterpretableRiskClusteringIntelligence(k=param['k'], fix=fix, distribution=param['distribution'],
                 layers=param['layers'], discount=param['discount'], num_units=num_units,
    ...
    br_y_train = combine_t_e(t_train, e_train)
    br_y_val = combine_t_e(t_val, e_val)
    br_y_test = combine_t_e(t_test, e_test)
    ...
    max_time_test = np.max([t[1] for t in br_y_test]) - 1
    
    print('max_time_test: ', max_time_test)
    # breastcancer -1, support - 0.00001, flchain - 0.00001, AV45 -1

    times, brier_score_test = brier_score(br_y_train, br_y_test, pred, max_time_test)
    brier_score_test = brier_score_test[0]

    return model, c_index, pred, pred_time, rae_nc, rae_c, brier_score_test


def test_IRIS(model, X_test, y_test):
