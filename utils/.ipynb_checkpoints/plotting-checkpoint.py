...
def plot_loss_c_index(results_all, lr, epoch, bs):
    ...
    plt.savefig('/sfs/gpfs/tardis/project/zhangmlgroup/fairness/github_codes/NAMDCSM/cindex_plots/loss_c_index_lr{}_epoch{}_bs{}.png'.format(lr, epoch, bs))
    plt.show()
    plt.close()
...
def get_feature_contributions(model, X, feature_names):
    ...
    for r in range(model.torch_model.risks): 
        ...
        for k in range(model.k):
            ...
            feature_contributions.append(feature_contributions_k)
                # if k == 0:
                #     print('feature', i, 'unique_feature', len(unique_feature), 'k', k, 'feat_contribution', len(feat_contribution), len(set(feat_contribution)))

            # exit(0)
    # for k in range(model.k):
    #     print(len(feature_contributions[k]))
    #     if k == 0:
    #         for i, _ in enumerate(feature_names):
    #             print(len(np.unique(X[:, i])), len(feature_contributions[k][i]))
    ...
    #             # if len(set(feat_contribution)) == 2:
    #             #     print('-------feature', i, 'k', k, 'feat_contribution', set(feat_contribution))
    #             # if k == 0:
    #             #     print('feature', i, 'unique_feature', len(unique_feature), 'k', k, 'feat_contribution', len(feat_contribution), len(set(feat_contribution)))
...
def calc_mean_prediction(model, X, feature_names):
    print('inside calc_mean_prediction')
    ...
    for k in range(model.k):
        ...
        # print(len(feature_contributions_k))
        ...
        # print('avg_hist_data:', type(avg_hist_data), len(avg_hist_data))
        
        all_indices_k, mean_pred_k = {}, {}

        # for i, col in enumerate(feature_names):
        #     feature_i = X[:, i]
        #     all_indices_k[col] = np.searchsorted(np.unique(feature_i), feature_i, 'left')
        # all_indices.append(all_indices_k)

        for col in feature_names:
            mean_pred_k[col] = np.mean([avg_hist_data_k[col]])  #[i] for i in all_indices[col]]) 
        
        mean_pred.append(mean_pred_k)

    # for k in range(model.k):
    #     avg_hist_data_k = avg_hist_data[k]
    #     for i, (name, feat_contrib) in enumerate(avg_hist_data_k.items()):
    #         print(len(np.unique(X[:, i])), len(feat_contrib))
...
def compute_mean_feature_importance(mean_pred, avg_hist_data):
    print('inside compute_mean_feature_importance')
    mean_abs_score = {}
...
def plot_mean_feature_importance(model, X, feature_names, seed, width=0.5):
    print('inside plot_mean_feature_importance')

    mean_pred_all_k, avg_hist_data_all_k = calc_mean_prediction(model, X, feature_names)
    # print('mean_pred:', len(mean_pred), 'avg_hist_data:', len(avg_hist_data))
    # exit(0)
    ...
    for k in range(model.k):
        ...
        importance_df.to_csv('/sfs/gpfs/tardis/project/zhangmlgroup/fairness/github_codes/NAMDCSM/nam_feature_importance/feature_importance_seed_{}_k_{}.csv'.format(seed, k), index=False)


        print('x1:', len(x1), 'x2:', len(x2))
        print('x1[0]:', len(x1[0]), x1[0])
        print('x2[0]:', x2[0].shape, x2[0])
        ...
        plt.savefig('/sfs/gpfs/tardis/project/zhangmlgroup/fairness/github_codes/NAMDCSM/nam_feature_importance_plots/mean_feature_importance_seed_{}_k_{}.png'.format(seed, k))
        plt.show()

    return fig

# plot_nams(model, X_test, feature_names, seed, num_cols=1)
def plot_nams(model, X, feature_names, seed, num_cols = 2, n_blocks = 20, color = [0.4, 0.5, 0.9], linewidth = 7.0, alpha = 1.0, feature_to_use = None):
    print('inside plot_nams')
    unique_feature = np.unique(X[:, 0])
    print('unique_feature:', len(unique_feature))
    ...
    for k in range(model.k):
        ...
        # print('mean_pred:', len(mean_pred), 'feat_data_contrib:', len(feat_data_contrib))
        # print('type(mean_pred):', type(mean_pred), 'type(feat_data_contrib):', type(feat_data_contrib))
        # first_key = next(iter(mean_pred))  # Get the first key
        # first_value = mean_pred[first_key]  # Get the corresponding value
        # print(f"Type of the first key: {type(first_key)}")
        # print(f"Type of the first value: {type(first_value)}")
        # first_key = next(iter(feat_data_contrib))
        # first_value = feat_data_contrib[first_key]
        # print(f"Type of the first key: {type(first_key)}")
        # print(f"Type of the first value: {type(first_value)}")
        # print(f"Length of the first value: {len(first_value)}")

        num_rows = len(feature_names) // num_cols
        ...
        # print('feat_data_contrib_pairs:', len(feat_data_contrib_pairs))
        feat_data_contrib_pairs.sort(key=lambda x: x[0])
        # for pair in feat_data_contrib_pairs:
        #     print(pair[0])

        mean_pred_pairs = list(mean_pred.items())
        mean_pred_pairs.sort(key=lambda x: x[0])

        sorted_feature_names = [pair[0] for pair in feat_data_contrib_pairs]
        sorted_indices = [list(feature_names).index(name) for name in sorted_feature_names]  # Get indices in original order
        X_sorted = X[:, sorted_indices]  # Reorder columns of X based on indices
        ...
        def shade_by_density_blocks(color: list = [0.9, 0.5, 0.5]):
            single_feature_data = X_sorted[:, i]
        ...
        for i, (name, feat_contrib) in enumerate(feat_data_contrib_pairs):
            ...
            unique_feat_data = np.unique(X_sorted[:, i])
            ax = plt.subplot(num_rows, num_cols, i + 1)

            # print('unique_feat_data:', len(unique_feat_data))
            # print('feat_contrib:', len(feat_contrib))
        ...
        plt.savefig('/sfs/gpfs/tardis/project/zhangmlgroup/fairness/github_codes/NAMDCSM/nam_plots/nam_plot_seed_{}_k_{}.png'.format(seed, k))
        plt.show()

        # exit(0)

def get_max_feature_contributions(model, X, feature_names):
    # print('X:', X.shape)
    # [64, 64, 4, 6, 64, 64, 64, 4]

    feature_contributions = []
    feature_contributions_max_k = [] # To store which k gives the max feat_contribution for each sample and feature
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

    for i, _ in enumerate(feature_names):
        # unique_feature = np.unique(X[:, i])
        feature = torch.tensor(X[:, i], dtype=torch.float32).to(device).reshape(-1, 1) 
        for r in range(model.torch_model.risks): 
            risk = str(r+1)
            feature_contributions_all_k = [] # Store all feat_contributions for current risk and feature
            feature_contributions_dict = {}
            for k in range(model.k):
                feature_nns = model.torch_model.alpha[risk][k]
                feat_contribution = feature_nns.feature_nns[i](feature).cpu().detach().numpy().squeeze()
                feature_contributions_all_k.append(feat_contribution)  # Store contributions for current k
                # if len(set(feat_contribution)) == 2:
                #     print('-------feature', i, 'k', k, 'feat_contribution', set(feat_contribution))
                # if k == 0:
                #     print('feature', i, 'k', k, 'feat_contribution', len(feat_contribution))
            
            # Now determine max k per sample
            feature_contributions_all_k = np.array(feature_contributions_all_k)  # Shape: (model.k, num_samples)
            max_values = np.max(feature_contributions_all_k, axis=0)  # Shape: (num_samples,)
            max_k_indices = np.argmax(feature_contributions_all_k, axis=0)  # Shape: (num_samples,)
        
            # print('feature_contributions_all_k:', feature_contributions_all_k.shape)
            
            # print(feature_contributions_all_k[:, :5])
            # print(max_values[:5])
            # print(max_k_indices[:5])

            for sample_idx, (k, value) in enumerate(zip(max_k_indices, max_values)):
                feature_contributions_dict[sample_idx] = {'k': k, 'value': value}

            # print('feature_contributions_dict:', len(feature_contributions_dict))
            # first_five_items = list(feature_contributions_dict.items())[:5]
            # for key, value in first_five_items:
            #     print(f"Key: {key}, Value: {value}")
            # exit(0)
        feature_contributions.append(max_values)
        feature_contributions_max_k.append(feature_contributions_dict)

    # print('feature_contributions_max_k:', len(feature_contributions_max_k), len(feature_contributions_max_k[0]))
    # print('feature_contributions:', len(feature_contributions), len(feature_contributions[0]), feature_contributions[0][0])

def get_feature_contributions_for_all_clusters(model, mean_pred, avg_hist_data, seed):
    x1, x2 = [], []

    for k in range(model.k):
        mean_pred_k = mean_pred[k]
        avg_hist_data_k = avg_hist_data[k]

        x1_k, x2_k = compute_mean_feature_importance(mean_pred_k, avg_hist_data_k)
        x1.append(x1_k)
        x2.append(x2_k)
