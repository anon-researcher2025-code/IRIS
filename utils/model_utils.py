...
from models.iris_torch import InterpretableRiskClusteringIntelligenceTorch
...
def plot_loss_c_index(results_all, lr, bs, k, dist, discount):
    ...
    plt.savefig('./cindex_plots/av45_SMOTE_loss_cindex_{}_{}_{}_IRIS.png'.format(lr, k, bs))
    plt.show()
    plt.close()


def pretrain_iris(model, t_train, e_train, t_valid, e_valid,
                 n_iter=10000, lr=1e-2, thres=1e-4):
    premodel = InterpretableRiskClusteringIntelligenceTorch(model.inputdim, 1,
                                         dist=model.dist,
                                         risks=model.risks,
...
def check_prediction(pred_train, pred_val):
    print('checking the prediction...')
    unique_pred_train = np.unique(pred_train)
    if len(unique_pred_train) == 1:
        print(f"Warning: All training predictions are the same: {unique_pred_train[0]}")
    unique_pred_val = np.unique(pred_val)
    if len(unique_pred_val) == 1:
        print(f"Warning: All validation predictions are the same: {unique_pred_val[0]}")

    variance_train = np.var(pred_train)
    variance_val = np.var(pred_val)

    if variance_train == 0:
        print("Warning: Model assigns the same prediction to all training samples.")

    if variance_val == 0:
        print("Warning: Model assigns the same prediction to all validation samples.")
...
#                                                         std=np.abs(float(premodel.scale[str(r + 1)])) / 10)
    ...
    for i in tqdm(range(n_iter)):
        ...
        # check_prediction(pred_train, pred_val)
        ...
        if i % 10 == 0:
            print('iter:', i)
            print('loss on train data: {:.4f}'.format(loss))
            print('loss on valid data: {:.4f}'.format(valid_loss))
            print('c index on train data: {:.4f}'.format(c_index_train))
            print('c index on valid data: {:.4f}'.format(c_index_valid))
