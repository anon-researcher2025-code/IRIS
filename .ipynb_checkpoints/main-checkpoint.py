...
def init_config():
    ...
    parser.add_argument('--dataset', type=str, default='coxnam',
...
param = {'learning_rate': 0.00007515105899297053, 'layers': [50, 50], 'k': 2,
        'iters': 1, 'distribution': 'Weibull', 'discount': 0.5}

# hold out testing with different splitting using the same parameters set
# for seed in [42, 73, 666, 777, 1009]:
for seed in [42]:

    X_train, X_test, y_train, y_test, column_names = load_data(args, random_state=seed)
    ...
    if args.train_DCSM:
        ...
        #     = train_test_DCSM(param, X_train, X_test, y_train, y_test, seed=seed, fix=True, method='DCSM')
        ...
        if fix:
            ...
            method = 'DSM'
        model, c_index, pred_DCSM, pred_time_DCSM, rae_nc_DCSM, rae_c_DCSM \
            = train_test_DCSM(param, X_train, X_test, y_train, y_test, column_names, seed=seed, fix=fix, method=method)
...
low_DCSM, high_DCSM = st.t.interval(confidence=0.95, df=len(result_DCSM)-1, loc=np.mean(result_DCSM), scale=st.sem(result_DCSM))
print('-----------------C Index results-----------------')
print('DCSM:{:.4f}±{:.4f} from {:.4f} to {:.4f}'.format(np.mean(result_DCSM), np.std(result_DCSM), low_DCSM, high_DCSM))
low_DCSM, high_DCSM = st.t.interval(confidence=0.95, df=len(logrank_DCSM)-1, loc=np.mean(logrank_DCSM), scale=st.sem(logrank_DCSM))
